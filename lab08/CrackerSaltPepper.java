...
public class CrackerSaltPepper {
  ...
  public static void main (String[] args) throws FileNotFoundException,
    ...
    boolean pepperFound = false;
    int pepper = 1;
    String hash, salt, user;
    for (Map.Entry<String, ArrayList<String>> pair : hash2unames.entrySet()) {
      ArrayList<String> stuff = pair.getValue();
      salt = stuff.get(0);
      user = stuff.get(1);
      // out.println("Trying user " + user);
      for (String p : commonPwds) {
        // Pepper search
        if (!pepperFound) {
          for (pepper = 1; pepper < 11; pepper++) {
            hash = md5hash(salt + pepper + p);
            if (hash2unames.containsKey(hash)) {
              // Found pepper
              pepperFound = true;
              out.println("Pepper found: " + pepper);
